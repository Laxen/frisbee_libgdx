...
public class TreeNEW extends Tile {
    private Body body;
    private CircleShape cs;
    private ChainShape chain;

    public TreeNEW(Texture texture, World world) {
        super(texture, 0, 0);
...
//        bdef.position.set((x + texture.getWidth() / 2) / GameVars.PPM, (y + texture.getHeight() / 2) / GameVars.PPM);
    ...
    public void init(float x, float y) {
        body.setTransform((x + getTexture().getWidth() / 2) / GameVars.PPM, (y + getTexture().getHeight() / 2) / GameVars.PPM, body.getAngle());
    }
